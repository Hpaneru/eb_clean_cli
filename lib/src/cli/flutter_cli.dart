...
class FlutterCli {
  ...
  static copyEnvs(Logger logger, String path) async {
    ...
    await _Cmd.run(cmd, ['.env.example', '.env-production'], workingDirectory: path);
    await _Cmd.run(cmd, ['.env.example', '.env-development'], workingDirectory: path);
    await _Cmd.run(cmd, ['.env.example', '.env-staging'], workingDirectory: path);
  ...
  static Future<void> runProject({String cwd = '.', String flavor = 'development'}) async {
    await _Cmd.start('flutter', ['run', '-t', 'lib/main-$flavor.dart', '--flavor', flavor], workingDirectory: cwd);
  }
